...
```
```{r Download a map for the USA: this is ~12 MB}
#####
...
```{r calculate 'as the crow flies distance}
#####
...
crow
```
```{r set up maps to calculate least cost distance}
#####
...
```{r calculate least cost path for populations 1 and 5}
#####
...
```{r calculate and draw all pairwise paths}
#####
...
```{r Calculate least cost path based on buffer around coastline}
#####
...
```{r plot the near-coast buffer zone}
